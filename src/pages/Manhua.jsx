...
const manhua = () => {
    ...
    useEffect(() => {
        async function getManhuaComic() {
            setValue({ ...value, isLoading: true })
            const response = await getManhua()
            setValue({ ...value, setManhua: response, isLoading: false })
        }
        getManhuaComic()
    }, [])
                                        ...
                                        value.setManhua && value.setManhua.slice(0, 50).map((manhua, i) => (
                                            <a href={`/details${manhua.endpoint}`} key={i} >
                                                <div className="group cursor-pointer">
                                                    ...
                                                    <p className='text-white text-sm font-medium py-1 md:py-2 group-hover:text-[#2693e6] duration-200'>{manhua.title.length > 20 ? manhua.title.slice(0, 20) + '...' : manhua.title}</p>
                                                </div>
                                            </a>
                                        ))
                                    )}
